...
class ResAttnBlock(nn.Module):
    def __init__(self, patch_num, dim, ced=64):
        super(ResAttnBlock, self).__init__()

        self.n = patch_num
        self.dim = dim

        self.transform = nn.Linear(ced, self.n * self.dim)
...
class ResUNet(nn.Module):
    def __init__(self, resolution=32, in_channels=1, std_cnl=32, ted=256, ced=64):
        ...
        self.attn_s3 = ResAttnBlock(patch_num=(resolution//4)**2, dim=std_cnl*4, ced=ced)

        self.resblock_bt = DownResBlock(in_channels=std_cnl*4, out_channels=std_cnl*8)
        self.attn_bt = ResAttnBlock(patch_num=(resolution//8)**2, dim=std_cnl*8, ced=ced)

        self.resblock_r_s3 = UpResBlock(in_channels=std_cnl*8, out_channels=std_cnl*4)
        self.attn_r_s3 = ResAttnBlock(patch_num=(resolution//4)**2, dim=std_cnl*4, ced=ced)
